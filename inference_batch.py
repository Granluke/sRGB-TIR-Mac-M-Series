...
def main():
    ...
    try:
        ...
        state_dict = pytorch03_to_pytorch04(torch.load(opts.checkpoint, map_location=device), opts.trainer)
        trainer.gen_a.load_state_dict(state_dict['a'])
        trainer.gen_b.load_state_dict(state_dict['b'])
    ...
    if opts.trainer == 'MUNIT':
        # Start testing
        style_fixed = Variable(torch.randn(opts.num_style, style_dim, 1, 1, device=device), volatile=False)
        for i, (images, names) in enumerate(zip(data_loader, image_names)):
            ...
            style = style_fixed if opts.synchronized else Variable(torch.randn(opts.num_style, style_dim, 1, 1, device=device), volatile=False)
